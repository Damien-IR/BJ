...
class N13460:
    def __init__(self):
        ...
        self.queue = deque()
        self.visited = [[[[False] * self.max_x for i in range(self.max_y)] for j in range(self.max_x)] for k in
                        range(self.max_y)]
        ...
        self.visited[self.red_pos[0]][self.red_pos[1]][self.blue_pos[0]][self.blue_pos[1]] = True
        self.queue.appendleft([self.red_pos, self.blue_pos, 0])

    def moved_pos(self, pos, move_pos):
        count = 0
        p = pos
        while self.game_map[add(p, move_pos)[0]][add(p, move_pos)[1]] != '#' \
    ...
    def get_answer(self):
        while len(self.queue) > 0:
            ...
            for d in self.move_able_dirs:
                is_red_end, next_red, red_count = self.moved_pos(pos=red_pos, move_pos=d)
                is_blue_end, next_blue, blue_count = self.moved_pos(pos=blue_pos, move_pos=d)
                if not is_blue_end:
                    if is_red_end:
                        return depth + 1
                    ...
                    if not self.visited[next_red[0]][next_red[1]][next_blue[0]][next_blue[1]]:
                        self.queue.appendleft([next_red, next_blue, depth + 1])
                        self.visited[next_red[0]][next_red[1]][next_blue[0]][next_blue[1]] = True
        return -1
