...
def dfs(pos: Tuple[int, int], visited: defaultdict, map_size: Tuple[int, int], map_data: list) -> int:
    if pos == (map_size[0] - 1, map_size[1] - 1):
        return 1
    result: int = 0
    for np in next_pos(pos=pos, map_size=map_size):
        if not visited[np] and compare(cur_pos=pos, after_pos=np, map_data=map_data):
            visited[np] = True
            result += dfs(pos=np, visited=visited, map_size=map_size, map_data=map_data)
            visited[np] = False
    return result


def next_pos(pos: Tuple[int, int], map_size: Tuple[int, int]) -> List[Tuple[int, int]]:
    result: list = []
...
def compare(cur_pos: Tuple[int, int], after_pos: Tuple[int, int], map_data: list) -> bool:
...
def solve():
    ...
    world_map: list = [list(map(int, stdin.readline().strip().split())) for _ in range(m)]
