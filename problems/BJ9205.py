...
def solution(s_pos: Tuple[int, int], e_pos: Tuple[int, int], conv_list: List[Tuple[int, int]]) -> str:
...
def bfs(s_pos: Tuple[int, int], e_pos: Tuple[int, int], conv_list: List[Tuple[int, int]]) -> bool:
...
for i in range(t):
    ...
    conv_positions: list = [tuple(map(int, stdin.readline().strip().split())) for _ in range(n)]
    end_pos: tuple = tuple(map(int, stdin.readline().strip().split()))
    print(solution(s_pos=start_pos, e_pos=end_pos, conv_list=conv_positions))
