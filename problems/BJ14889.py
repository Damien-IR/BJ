...
def solution(size: int, stats: list):
    ...
    for i in range(len(combs) // 2 + 1):
        ...
        result = min(result, abs(total_stats(a_team, stats) - total_stats(b_team, stats)))
    return result


# (구) 코드의 잔재
# def dfs(team: set, size: int, stats: list, visited: defaultdict):
#     result: int = 1000000
#     if len(team) >= size // 2:
#         other_team: set = set(range(size)).difference(team)
#         if not visited[frozenset(team)]:
#             visited[frozenset(team)] = True
#             visited[frozenset(other_team)] = True
#             result = min(result, abs(total_stats(team, stats) - total_stats(other_team, stats)))
#         return result
#     for i in range(size):
#         if i not in team:
#             team.add(i)
#             result = min(result, dfs(team, size, stats, visited))
#             team.remove(i)
#     return result
...
def solve():
    ...
    map_data: list = [list(map(int, stdin.readline().strip().split())) for _ in range(n)]
