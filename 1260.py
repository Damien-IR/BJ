...
def solution(num: int, arr: list, input_v: int) -> tuple:
    ...
    for value in node_dict.values():
        value.sort()
    dfs(node=node_dict, visit=dfs_visited, cur_vertex=input_v, result=dfs_result)

    bfs_result = bfs(num=num, node=node_dict, vertx=input_v)
    return ' '.join(list(map(str, dfs_result))), ' '.join(list(map(str, bfs_result)))
...
nodes = [[int(i) for i in stdin.readline().strip().split()] for j in range(m)]
